...
export default function GameBoard({ sessionId, userId, playerColor }: Props) {
  ...
  const { data: stateRaw, refetch } = trpc.game.getGameState.useQuery(
    { sessionId },
    { refetchInterval: gameStatus === 'active' ? 500 : 2000 }
  );
  ...
  useEffect(() => {
    ...
    } else if (gameStatus === 'finished' && state?.finishedAt) {
  ...
  // Обновление состояния из БД (только серверные данные)
  useEffect(() => {
    if (state) {
  ...
  }, [currentNumber, gameStatus, winner, players, sessionId, makeBotMove, refetch]);
      ...
      <div style={{ maxWidth: '1200px', margin: '40px auto', padding: '0 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '250px 1fr 250px', gap: '24px' }}>
          ...
          <div style={{ background: '#101528', borderRadius: '20px', padding: '24px' }}>
            {waitingForPlayers ? (
              <div style={{ textAlign: 'center', padding: '60px', color: '#9ca3af' }}><div style={{ fontSize: '48px' }}>🕐</div><div>Ожидание игроков... ({humanPlayers.length}/{maxPlayers})</div></div>
            ) : isFinished && winner ? (
              <div style={{ textAlign: 'center', padding: '60px' }}>
                <div style={{ fontSize: '64px', marginBottom: '16px' }}>🏆</div>
                <h2 style={{ color: 'white', marginBottom: '8px' }}>{String(winner) === String(userId) ? 'Вы победили!' : `Победил: ${winner === null ? 'Бот' : winner.slice(0, 8)}`}</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px' }}>
                {grid.flat().map((num: number, idx: number) => {
                  ...
                  return (
                    <button key={idx} onClick={() => handleCellClick(num)} disabled={isDisabled} style={{
                      aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center',
                      background: cellColor ? bgColor : '#1a1f33', borderRadius: '12px', fontSize: '20px', fontWeight: 'bold',
                      color: cellColor ? 'white' : '#E8EAFF', border: isCurrent && !cellColor && gameStatus === 'active' ? '3px solid #F5A623' : '1px solid #2a2f45',
                      cursor: isDisabled ? 'not-allowed' : 'pointer', opacity: isDisabled ? 0.6 : 1,
                      boxShadow: isCurrent && !cellColor && gameStatus === 'active' ? '0 0 15px rgba(245,166,35,0.6)' : 'none', transition: 'all 0.05s ease'
                    }}>{num}</button>
                  );
                })}
              </div>
